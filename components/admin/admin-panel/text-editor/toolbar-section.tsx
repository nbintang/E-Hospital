...
import type { FormatAction } from '@/types/tiptap'
...
import { getShortcutKey } from '@/lib/tiptap-utils'
  ...
  const renderToolbarButton = React.useCallback(
    (action: FormatAction) => (
      <ToolbarButton
        ...
        tooltip={`${action.label} ${action.shortcuts.map((s : any) => getShortcutKey(s).symbol).join(' ')}`}
